	...
	constructor(vf_bars, beat_value, beats_per_measure, bpm) {
		...
		for (let i = 0; i < vf_bars.length; i++) {
			...
			for (let j = 0; j < notes.length; j++) {
				...
				time_offset += percentage * beats_per_measure * (60 / bpm) * 1000
				accumuluated_percented += percentage;
				if (j !== notes.length - 1) {
					this.note_percentages[i].push({note: notes[j + 1], percentage: accumuluated_percented, time_offset: time_offset});
				}
			}
		}
	}
